...
`HughesNet.Files` is a library for .NET Standard 2.1 that facilitates reading and writing various file formats, such as CSV, Excel, TXT, XML, HTML and ZIP. It abstracts the complexity of underlying libraries and provides a simple and unified API for working with files.
...
```powershell
Install-Package HughesNet.Files
```

Or via the .NET CLI:

```bash
dotnet add package HughesNet.Files
```
...
```csharp
using HughesNet.Files;
using HughesNet.Base;
