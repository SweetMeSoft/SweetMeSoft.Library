# HughesNet.Connectivity

Library to simplify and standardize requests to APIs and web services.

## Description

`HughesNet.Connectivity` is a library for .NET Standard 2.1 that provides an `ApiReq` class as a wrapper over `HttpClient`. Its goal is to facilitate making HTTP requests (GET, POST, PUT, DELETE), handling different content types, cookie management and response deserialization in a structured and reusable way.
...
```bash
dotnet add package HughesNet.Connectivity
```
