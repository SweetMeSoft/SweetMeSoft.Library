...
`HughesNet.GCP` is a library for .NET Standard 2.1 that simplifies interaction with Google Cloud services, specifically with **BigQuery** and **Cloud Storage**. It provides two main classes, `BigQueryRepo` and `GCPStorage`, which act as repositories to facilitate common operations.
