# HughesNet.Middleware

Library with a set of Middlewares and services for ASP.NET Core applications.

## Description

`HughesNet.Middleware` is a library for .NET Standard 2.1 that offers components for ASP.NET Core, designed to centralize error management, log requests and facilitate access to services through a Factory pattern.
...
```bash
dotnet add package HughesNet.Middleware
```
...
```csharp
using HughesNet.Middleware;
using HughesNet.Middleware.Interface;
using HughesNet.Middleware.Service;
