...
Base library with a set of utilities, helpers and fundamental classes for HughesNet projects.

## Description

`HughesNet.Base` is a library for .NET Standard 2.1 that serves as the cornerstone for other HughesNet libraries and applications. It provides a set of reusable components that include custom attributes, generic connectivity, captcha handling, application constants, file helpers, Google Cloud Platform (GCP) utilities and email handling tools.
...
Base classes and enums for file manipulation, used by `HughesNet.Files`:
...
```powershell
Install-Package HughesNet.Base
```

Or via the .NET CLI:

```bash
dotnet add package HughesNet.Base
```
